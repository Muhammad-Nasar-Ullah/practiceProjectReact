...
const Navbar = ({ mode, toggleMode }) => {
    ...
    return (
        <div className={`px-5 py-5 relative flex justify-between items-center sticky top-0 h-[70px] z-50 transition-all ${!dropdown ? 'delay-250 duration-500' : 'delay-0 duration-300'} ${mode === 'dark' ? 'bg-gray-800' : 'bg-gray-200'}`}>
            ...
            <div className={`flex flex-col gap-5 fixed top-0 w-full h-[100vh] z-[100] p-5 shadow-lg border transition-all ease-in-out ${!dropdown ? 'delay-250 duration-300 ' : 'delay-0 duration-300'} ${dropdown ? 'right-0' : '-right-full'} ${mode === 'dark' ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-800'}`}>
                <button onClick={() => setDropdown(false)} className={`absolute top-3 right-3 text-sm px-2 py-2 rounded-md transition ${mode === 'dark' ? 'bg-gray-700 hover:bg-gray-600 text-white' : 'bg-gray-200 hover:bg-gray-300 text-black'}`}>
                ...
                <button onClick={toggleMode} onMouseDown={() => setDropdown(false)} className={`w-full py-2 rounded-lg font-medium transition ${!dropdown ? 'delay-250 duration-200' : 'delay-0 duration-300'} ${mode === 'dark' ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-800'}`}>
