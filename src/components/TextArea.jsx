...
const TextArea = ({ mode }) => {
    ...
    return (
        <div className='flex flex-col gap-5 w-[90%] md:w-[60%] lg:w-[50%] mx-auto my-5 rounded-lg'>
            ...
            <h2 className={`text-2xl font-bold transition-all duration-500 ${mode === 'dark' ? 'text-white' : 'text-black'}`}>Enter Your Text Here</h2>
            ...
            <p className={`${mode === 'dark' ? 'text-white' : 'text-black'} transition-all duration-500`}>{totalwords} {totalwords < 2 ? 'word' : 'words'} {text.length} {text.length < 2 ? 'character' : 'characters'}</p >
            <p className={`${mode === 'dark' ? 'text-white' : 'text-black'} transition-all duration-500`}>{min} {min < 2 ? 'minute' : 'minutes'} {sec} {sec < 2 ? 'second' : 'seconds'} to read</p>
            <p className={`whitespace-pre-wrap transition-all duration-500 ${mode === 'dark' ? 'text-white' : 'text-black'}`}>{text.length > 0 ? text : 'Enter your text to preview here'}</p>
